...
=== Method map — where each table is used ===
    ...
    unit_outlines          -> save_doc, get_doc, get_doc_by_unit_code, delete_doc  (wired to editing/routes.py)
    history_log            -> log_history                                          (wired to editing/routes.py)
...
The unit_outlines/history_log methods ARE wired up now — the frontend's
"Save Outline Changes" button posts to editing/routes.py, which calls
SaveDoc (editing.py), which calls save_doc() + log_history() below.
The outline content shape that was still undecided is settled: it's a
JSON object of the Unit Outline form's field ids, serialised to a
string — see SaveDoc._content_json() in editing.py.
...
class Retrieve_Database:
    ...
    @classmethod
    def get_doc_by_unit_code(cls, unit_code: str) -> UnitOutlineRecord | None:
        """
        Look up an outline document by its unit code (e.g. "ICT101")
        rather than its numeric ID.

        This is what the Save Outline Changes button needs: the frontend
        knows which UNIT it's saving, not which unit_outlines row. If
        this returns a record, save_doc() should be called with that
        record's .id (so it saves a new version of the same row); if it
        returns None, the unit has never been saved before and save_doc()
        should be called with unit_outline_id=None (so it inserts).

        If the same unit_code somehow has more than one row, the most
        ...
        try:
            ...
            cursor.execute(
                "SELECT unit_outline_id AS id, unit_code, content, last_edited_by, last_edited_at, version "
                "FROM unit_outlines WHERE unit_code = %s "
                "ORDER BY unit_outline_id DESC LIMIT 1",
                (unit_code,),
            )
            row = cursor.fetchone()
        finally:
