...
from email_approval.email_service import send_approval_email, EmailSendError
from editing.editing import ApprovalReq

email_bp = Blueprint("email", __name__)

# Deliberately permissive — enough to catch "test" or a missing @, not
# an attempt to fully validate email addresses (which is a famously
# unwinnable game). The mail server is the real judge.
...
def api_create_change_request():
    ...
    # Checked here, not only in the browser: the frontend's input is
    # type="email" but it isn't inside a <form>, so nothing was actually
    # validating it — "test" was accepted and then handed straight to
    # the mail server, which rejected it with a confusing SMTP error.
    if not EMAIL_PATTERN.match(coordinator_email):
        return jsonify({"error": f"'{coordinator_email}' is not a valid email address"}), 400
    ...
    try:
        ...
        # The approval record exists but no email went out. Say exactly
        # that, and pass the real reason through so the user can act on
        ...
        return jsonify({"error": f"Could not create the change request: {err}"}), 500
