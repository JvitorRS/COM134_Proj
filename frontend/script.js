/* ============================================================
   ...
   The course list is real: it comes from the `courses` table in
   MySQL via GET /dashboard/api/courses (dashboard.py). Each course's
   outline CONTENT comes from the `unit_outlines` table via
   GET /editing/api/outlines/<unit_code> (editing/routes.py).

   localStorage is used only as a cache of what the server last said,
   so the UI can render instantly and the version-history side panel
   has something to diff against. It is never the source of truth.
   ============================================================ */

const API_BASE_URL = ""; // e.g. "http://localhost:3000/api"

/* Whether the last attempt to load courses from the database actually
   succeeded. renderCourses() uses this to tell the difference between
   "you have no courses" and "we couldn't reach the database" — those
   need very different messages, and showing the first when the second
   is true is how a broken connection gets mistaken for an empty one. */
let backendCoursesLoaded = false;

/* There is deliberately no `defaultCourses` array any more.

   It used to hold three sample courses (CS101 / IT201 / AI301) that
   getCoursesStore() fell back to whenever localStorage was empty. That
   fallback is why the dashboard kept showing those three even when
   nothing was reaching MySQL: a failed database call looked exactly
   like a working dashboard, and the sample data then got saved into
   localStorage as if it were real, so it persisted afterwards. If you
   want starting courses, insert them in the database (there are sample
   INSERTs at the bottom of "SPI Unit Outline Systems.sql") or add them
   with the Add Course button. */
...
/* ============================================================
   ...
   Both halves are real now:
     - the LIST of courses (code/name/semester/start date/description)
       comes from GET /dashboard/api/courses
     - each course's outline CONTENT comes from
       GET /editing/api/outlines/<unit_code>

   syncCoursesFromBackend() merges the server's course list into the
   local cache; hydrateOutlineFromBackend() fills in a course's outline
   content when it's opened. The local copy is a cache of those two,
   nothing more.
   ============================================================ */

function getCoursesStore() {
    const existing = localStorage.getItem("spi_courses");
    // Empty means "nothing cached yet", which is the honest answer.
    // This used to seed three hardcoded sample courses instead, which
    // made an unreachable database look like a populated dashboard.
    if (!existing) return [];
    ...
    try {
        ...
        // Corrupted cache (hand-edited, or a half-written value): drop
        // it rather than letting every caller throw on the same bad JSON.
        console.error("Cached course list was not valid JSON — clearing it:", err);
        localStorage.removeItem("spi_courses");
        return [];
    }
}
...
async function loadDashboard() {
    ...
    // The course list comes from the database, via
    // GET /dashboard/api/courses. If that fails, say so plainly rather
    // than quietly showing three hardcoded sample courses — that was
    // the old behaviour, and it made a broken database connection look
    // like a working dashboard, which is how CS101/IT201/AI301 kept
    // appearing when nothing was actually reaching MySQL.
    try {
        await syncCoursesFromBackend();
        backendCoursesLoaded = true;
    } catch (err) {
        console.error("Could not load courses from the backend:", err);
        backendCoursesLoaded = false;
        showToast(`Could not load courses from the database — ${err.message}`, "error");
    }
...
function renderCourses(courses) {
    ...
    if (!courses.length) {
        // Two very different situations, two different messages. Telling
        // someone "No courses yet" when the database is actually
        // unreachable sends them looking for the wrong problem.
        grid.innerHTML = backendCoursesLoaded
            ? `<div class="empty-state">
                 <h3>No courses yet</h3>
                 <p>Add a course to begin managing its unit outline.</p>
               </div>`
            : `<div class="empty-state">
                 <h3>Could not load courses from the database</h3>
                 <p>The course list lives in the <code>courses</code> table in MySQL.
                    Check that the app is running with <code>DB_HOST</code>,
                    <code>DB_USER</code>, <code>DB_PASSWORD</code> and
                    <code>DB_NAME</code> set in its <code>.env</code> file, then reload.</p>
               </div>`;
        return;
    }
...
async function submitChangeRequest() {
    ...
    // The input is type="email" but it isn't inside a <form>, so the
    // browser never validates it on its own. Without this check, "test"
    // was sent to the server and came back as a confusing mail error.
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(approverEmail)) {
        showToast(`"${approverEmail}" is not a valid email address`);
        return;
    }
    ...
    try {
        ...
        if (!response.ok) {
            // The server sends {"error": "..."} with the actual reason —
            // missing mail credentials, a bad address, SMTP refusing the
            // login. Show that instead of a guess about the server being
            // down, which was almost never the real cause.
            let reason = `Server responded ${response.status} ${response.statusText}`;
            try {
        ...
        showToast(`Could not send the approval email — ${err.message}`, "error");
        return;
    }
...
function openCourse(courseId) {
    ...
    // Then overlay whatever the DATABASE has for this unit, so the
    // outline is the same on every machine instead of only on the
    // browser it was typed in. Deliberately last and not awaited: the
    // form is already usable from the local copy above, and this just
    // corrects it a moment later if the server has something newer.
    hydrateOutlineFromBackend(course);
}

/* Pulls this course's saved outline out of the database and drops it
   into the form, replacing the local copy. Silent if the unit has never
   been saved (a brand-new course keeps showing the blank template) or
   if the server can't be reached — the locally-loaded form stays as-is
   in both cases, which is what the user already has in front of them. */
async function hydrateOutlineFromBackend(course) {
    let record;
    try {
        record = await loadOutlineFromBackend(course.code);
    } catch (err) {
        console.error("Could not load the saved outline from the database:", err);
        return;
    }
    if (!record) return;

    // The user may have navigated elsewhere (or opened a different
    // course) while this request was in flight — don't stamp one
    // course's data onto another course's form.
    const stillActive = JSON.parse(localStorage.getItem("spi_active_course") || "null");
    if (!stillActive || stillActive.id !== course.id) return;
    if (previewingVersion) return;   // they're reading a historical version; leave it alone

    applyOutlineData(record.outlineData);
    document.getElementById("unit_code").value = record.outlineData.unit_code || course.code;
    document.getElementById("title").value = record.outlineData.title || course.name;
    document.getElementById("disp_organisation").textContent =
        record.outlineData.organisation || "Sydney Polytechnic Institute";
    updateRolePermissions();

    // Cache it locally too, so the next diff/version snapshot compares
    // against what's actually in the database. Written straight to the
    // store rather than through saveCourseChanges() on purpose — this
    // is loading, not editing, and it must not create a version entry.
    const courses = getCoursesStore();
    const index = courses.findIndex(c => c.id === course.id);
    if (index !== -1) {
        courses[index].outlineData = record.outlineData;
        setCoursesStore(courses);
        renderVersionSidePanel(courses[index]);
    }
}
...
/* Sends the outline to the backend, which writes it into the
   unit_outlines table and records the save in history_log.
   POST /editing/api/outlines/<unitCode>  (editing/routes.py)

   Returns the saved record so the caller can show its version number.
   Throws if the server rejects the save — the caller decides what the
   user sees. */
async function saveOutlineToBackend(unitCode, outlineData) {
    const response = await fetch(`/editing/api/outlines/${encodeURIComponent(unitCode)}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "same-origin",
        body: JSON.stringify({ outlineData }),
    ...
    if (!response.ok) {
        // The backend sends {"error": "..."} for every failure case
        // (not logged in, no edit access, DB down) — surface its actual
        // reason rather than a generic "save failed".
...
/* Loads a unit's saved outline back out of the database.
   GET /editing/api/outlines/<unitCode>
   Returns null when the unit has never been saved (the backend answers
   404 for that, which is a normal state, not an error). */
async function loadOutlineFromBackend(unitCode) {
    const response = await fetch(`/editing/api/outlines/${encodeURIComponent(unitCode)}`, {
        credentials: "same-origin",
    });
    ...
    return record && record.outlineData ? record : null;
}
...
async function saveChanges() {
    ...
    const active = JSON.parse(localStorage.getItem("spi_active_course") || "null");
    if (!active) {
        showToast("No unit outline is open");
        return;
    }

    const outlineData = collectOutlineData();

    // Save locally FIRST so the version-history side panel and the
    // in-browser copy stay correct even if the network call fails —
    // the user's typing is never lost to a failed request.
    saveCourseChanges(active.id, outlineData);
    const refreshed = getCoursesStore().find(c => c.id === active.id);
    if (refreshed) renderVersionSidePanel(refreshed);

    // The outline is filed in the database under the UNIT CODE, not the
    // numeric course id — the code is what unit_outlines.unit_code holds.
    const unitCode = document.getElementById("unit_code").value.trim() || active.code;
    ...
    try {
        const record = await saveOutlineToBackend(unitCode, outlineData);
        showToast(`Unit Outline saved to the database (version ${record.version})`);
    } catch (err) {
        console.error("Could not save the outline to the database:", err);
        // Say plainly that the DATABASE save failed — the local copy
        // did save, so "saved" on its own would be misleading.
        showToast(`Saved in this browser, but NOT to the database — ${err.message}`);
    } finally {
        if (saveBtn) {
...
/* Export All: downloads a ZIP holding ONE JSON FILE PER COURSE
   (CS101.json, IT201.json, ...) built by the backend from the database
   — see /dashboard/api/courses/export in dashboard.py.

   This used to produce a single spi-courses.json containing every
   course in one array, built from the localStorage cache.

   A ZIP rather than one download per course because a page isn't
   allowed to start several downloads by itself: Chrome interrupts with
   a "Download multiple files?" prompt and other browsers drop all but
   the first. One ZIP always arrives, and extracting it gives the
   separate files. */
async function exportAllCourses() {
    showToast("Preparing export…");

    try {
        const response = await fetch("/dashboard/api/courses/export", {
            credentials: "same-origin",
        });
        ...
        // Use the filename the server chose (it carries the date) when
        // the browser exposes it; otherwise fall back to a sensible one.
        let filename = "spi-courses.zip";
        ...
        showToast("Courses exported — one JSON file per course inside the ZIP");
    } catch (err) {
        console.error("Could not export the courses:", err);
        showToast(`Could not export the courses — ${err.message}`, "error");
    }
}

/* Import JSON: reads a course file and creates the courses in the
   DATABASE, via POST /dashboard/api/courses.

   Two things were wrong before:

   1. It only wrote to localStorage. Because the dashboard re-syncs from
      the database on every load, anything imported vanished on the next
      refresh — it looked like the button had done nothing.

   2. Every failure reported the same "invalid JSON file", whatever the
      real cause. That message was also shown for files that were
      perfectly valid JSON but not course data, and it never said WHERE
      the problem was, which is the one thing you need to fix it.

   Accepts a single course object, an array of them, or a file from
   Export All (which carries extra outline fields — those are ignored
   here; outline content is saved through the outline screen). */
async function importCourseData(file) {
    if (!file) return;

    // Let the same file be picked again later. Without this, choosing
    // the same filename twice in a row fires no change event, so the
    // button appears dead after a failed import.
    ...
    try {
        ...
        // SyntaxError from JSON.parse names the exact position of the
        // problem, e.g. "Unexpected non-whitespace character after JSON
        // at position 4782". Passing that through turns a dead end into
        // something you can actually go and fix — a stray character at
        // the end of the file being the classic one.
        console.error("Import failed while parsing JSON:", err);
        showToast(`${file.name} is not valid JSON — ${err.message}`, "error");
        if (input) input.value = "";
        return;
    }
    ...
    // Keep only entries that actually look like courses, and say how
    // many were skipped rather than silently importing nothing.
    const valid = candidates.filter(validateCourseData);
    const skipped = candidates.length - valid.length;

    if (!valid.length) {
        showToast(
            `No courses found in ${file.name} — each course needs at least a "code" and a "name"`,
            "error"
        );
        if (input) input.value = "";
        return;
    ...
    showToast(`Importing ${valid.length} course(s)…`);
    ...
    for (const course of valid) {
        try {
            ...
            console.error(`Could not import course ${course.code}:`, err);
            failures.push(`${course.code} (${err.message})`);
        }
    }

    // Re-read from the database so the dashboard shows what was really
    // saved, not what we hoped was saved.
    try {
        await syncCoursesFromBackend();
        backendCoursesLoaded = true;
    } catch (err) {
        console.error("Imported, but could not re-read the course list:", err);
    }
    loadDashboard();
    ...
    if (failures.length) {
        showToast(
            `Imported ${created} of ${valid.length} course(s). Failed: ${failures.join("; ")}`,
            "error"
        );
    } else {
        showToast(
            `${created} course(s) imported into the database` +
            (skipped ? ` — ${skipped} entr${skipped === 1 ? "y" : "ies"} skipped (not course data)` : "")
        );
    }
}
...
/* showToast(message, type)
   ...
   `type` is "info" (default) or "error". Error toasts are styled red
   and stay up much longer: they now carry real diagnostic text (a
   database error, a mail server's refusal, the exact position of a
   JSON syntax error), and 2.6 seconds isn't enough to read a sentence
   like that, let alone act on it.

   The second argument used to be accepted and ignored — showMessage()
   below has always passed one. */
...
/* The old "BACKEND CONNECTION (Python <--> JavaScript)" demo section
   used to live here: BACKEND_ORIGIN, backendUrl(), BACKEND_URL,
   loadUnitsFromBackend() and renderBackendUnits(), which filled the
   "Units from Python backend" table by calling GET /api/units.

   Removed, along with that table in index_1.html. It was a teaching
   demo from the original codebase, not part of this app: /api/units
   has never existed in this Flask app, so it only ever showed a red
   "could not reach backend" error, and it duplicated the My Courses
   cards above it, which show the same course data from the real
   endpoint (GET /dashboard/api/courses).

   unit.js, which only defined the Unit class that table used, is no
   longer loaded by index_1.html either. */
